...
<script>
  ...
  export default {
    ...
    data() {
      return {
        helper,
        forminfo: {...PILE.FORM_INFO},
        formdata: {...PILE.FORM_DATA},
        formOptions: Object.assign(PILE.FORM_OPTIONS, {
        }),
        rules: PILE.RULES,
        windowOptions: {
          title: null,
    ...
    methods: {
      open(row) {
        if (row) {
          this.windowOptions.title = '编辑充电桩'
          this.formdata = {...row}
          this.$refs.window.open()
        } else {
          this.$refs.window.open()
          this.$nextTick(() => {
            this.windowOptions.title = '新增充电桩'
            this.$refs.form.resetFields()
            this.formdata = {...PILE.FORM_DATA}
          })
        }
      },
      sure() {
        this.$refs.form.validate((valid) => {
          if (valid) {
            if (!this.formdata.id) {
              api.pile.add(this.formdata).then(() => {
                this.$message({
                  message: '新增成功!',
                  type: 'success'
                })
              ...
              api.pile.update(this.formdata).then(() => {
                this.$message({
                  message: '编辑成功!',
                  type: 'success'
                })
