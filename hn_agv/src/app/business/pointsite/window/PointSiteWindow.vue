...
<script>
  ...
  export default {
    ...
    data() {
      return {
        helper,
        forminfo: {...POINT.FORM_INFO},
        formdata: {...POINT.FORM_DATA},
        formOptions: Object.assign(POINT.FORM_OPTIONS, {
        }),
        rules: POINT.RULES,
        windowOptions: {
          title: null,
    ...
    methods: {
      open(row) {
        if (row) {
          this.windowOptions.title = '编辑地图点位'
          this.formdata = {...row}
          this.$refs.window.open()
        } else {
          this.$refs.window.open()
          this.$nextTick(() => {
            this.windowOptions.title = '新增地图点位'
            this.$refs.form.resetFields()
            this.formdata = {...POINT.FORM_DATA}
          })
        }
      },
      sure() {
        this.$refs.form.validate((valid) => {
          if (valid) {
            if (!this.formdata.id) {
              api.pointSite.add(this.formdata).then(() => {
                this.$message({
                  message: '新增成功!',
                  type: 'success'
                })
              ...
              api.pointSite.update(this.formdata).then(() => {
                this.$message({
                  message: '编辑成功!',
                  type: 'success'
                })
