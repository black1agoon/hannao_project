...
<script>
  import api from '@/api'
  import {ONOFF} from './storage.config'
  import helper from '@/app/app.helpers'
  import EuiLayout from '@/components/common/layout/EuiLayout'
  import EuiTable from '@/components/common/form/EuiTable'
  import EuiInput from '@/components/common/form/EuiInput'
  export default {
    name: 'storage-on-off-detect',
    _meta: {
      path: '/storage/onoffdetect',
      title: '仓库上下线管理'
    },
    components: {
      EuiLayout,
    ...
    data() {
      return {
        ...
        tableOptions: {
          ...
          total: 0,
          columns: [
            {prop: 'location', label: '库位位置', minWidth: 150},
    ...
    methods: {
      getData() {
        api.storageOnoff.paged(this.query).then(data => {
