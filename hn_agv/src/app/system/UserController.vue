    ...
    methods: {
      ...
      getTreeData() {
        api.department.getTree().then((data) => {
          this.treeOptions.data = this.formOptions.depId = this.searchOptions.conditionSearch.find(c => c.value === 'depId').options = helper.getTreeData(data, 'childDepartmentDtoList', 'name')
        })
      },
      ...
      getRolesData() {
        api.role.getPaged({
          ...
          this.searchOptions.conditionSearch.find(c => c.value === 'roleId').options = data.records.map(d => ({
            label: d.role,
            value: d.id
          }))
        })
      ...
      getInputData() {
        api.dictionary.getValuePaged({
          ...
          this.formOptions.sex = this.searchOptions.conditionSearch.find(c => c.value === 'sex').options = data.records.map(d => ({
            label: d.name,
            value: d.code
          }))
        })
      ...
      superSearch(searchinfo) {
        ...
        this.query.sex = searchinfo.sex
        this.query.depId = searchinfo.depId
        this.query.roleId = searchinfo.roleId
        this.getData(this.query)
      },
      emptySearch(tagname) {
