<template>
  <div class="form-wrapper">
              ...
              <div v-else-if="item.value === 'banner'">
                <div class="piclist">
                  <div class="pic-wrapper" v-for="(pic, index) in formdata.banner" :key="index">
                    ...
                    <el-input v-if="!!pic" v-model="pic.seq" type="number" placeholder="排序"></el-input>
                  </div>
                </div>
    ...
    <product-brand-chose-window ref="brandWindow" :company-id="formdata.companyId">
    ...
    data() {
      return {
        ...
        formOptions: Object.assign({
          thumb: {
            uploadFile: (params) => {
              api.public.upload4CompanyPic(params.file, true).then(data => {
        ...
        uploadPic: api.public.upload4CompanyPic,
    ...
    methods: {
      submitForm() {
        this.$refs.form3.validate((valid) => {
          if (valid) {
            if (!this.formdata.id) {
              api.product.addProduct(this.formdata).then(() => {
                this.$message({
                  message: '新增成功!',
                  type: 'success',
                  duration: 1000,
              ...
              api.product.updateProduct(this.formdata).then(() => {
                this.$message({
                  message: '编辑成功!',
                  type: 'success',
                  duration: 1000,
      ...
      getProductInput() {
        api.productType.getTypeTree().then(data => {  // 获取产品类型
          this.formOptions.typeId = helper.getTreeData(data && data.length > 0 ? data : [], 'childProductTypeList', 'name')
        })
        this.getProductCategory()
      },
      getProductCategory(firstEnter = true) {
        api.productCategory.getCategoryShowPaged({
          ...
          if (this.formOptions.catId.length > 0) {
            if (firstEnter) {
              this.formdata.catId = this.formdata.catId ? this.formdata.catId : this.formOptions.catId[0].value
            } else {
              this.formdata.catId = this.formOptions.catId[0].value
            }
            this.getProductAttr(this.formdata.catId)
          } else {
            this.formOptions.catId = []
            this.formdata.catId = null
            this.getProductAttr(this.formdata.catId)
          }
        })
      },
      getProductAttr(categoryId) {
        api.productCategoryAttr.getAttrPaged({
      ...
      getData(id) {
        return api.product.getProduct(id).then(data => {
          this.formdata = data
    ...
    created() {
      ...
      this.$on('uploadlists', (lists, fromname) => {
        if (fromname === 'banner') {
          ...
          this.formdata.reports = this.formdata.reports.concat(lists)
        }
      })
    },
    watch: {
      'formdata.companyId'() {
        if (!this.firstEnter) {
          this.getProductCategory(this.firstEnter)
        }
      ...
      .form2
        ...
        .video-player
          ...
          .delete
            ...
            text-align: right
            vertical-align: top
    .el-input, .el-cascader, .el-select, .el-textarea, .vue-treeselect
      width: 400px
