...
<script>
  ...
  import {TEMPLATE} from './product.config'
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          route: [{
            required: true, message: '请输入模板路径', trigger: ['blur', 'change']
          }]
        },
        query: {
          name: null,
        ...
        tableOptions: {
          ...
          columns: [
            ...
            {prop: 'route', label: '路径', minWidth: 200},
    ...
    methods: {
      getData(query) {
        api.productTemplate.getPaged(query).then(data => {
        ...
        let btnHandlers = {
          btnAdd: () => {
            ...
            this.$nextTick(() => {
              ...
              this.formdata = {...TEMPLATE.FORM_DATA}
            })
          },
          btnEdit: (arg) => {
          ...
          btnDelete: (arg) => {
            ...
            this.$confirm(`是否删除:${this.formdata.name}？`, '提示', {
              ...
              api.productTemplate.delete(this.formdata.id).then(() => {
                this.$message({
                  message: '删除成功!',
                  type: 'success'
                })
      ...
      sure() {
        this.$refs.form.validate((valid) => {
          if (valid) {
            if (!this.formdata.id) {
              api.productTemplate.add(this.formdata).then(() => {
                this.$message({
                  message: '新增成功!',
                  type: 'success'
                })
              ...
              api.productTemplate.update(this.formdata).then(() => {
                this.$message({
                  message: '编辑成功!',
                  type: 'success'
                })
