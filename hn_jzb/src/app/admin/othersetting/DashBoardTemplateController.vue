<template>
  <eui-layout :regions="REGIONS">
    ...
    <eui-table
      ...
      <el-table-column label="是否默认" minWidth="200" align="center" fixed="right">
        <template slot-scope="scope">
          <el-switch :value="Boolean(scope.row.isDefault)"
                     @change="changeIsDefault(scope.$index,scope.row)">
          </el-switch>
        </template>
      </el-table-column>
...
<script>
  ...
  import {DASHBOARD} from './othersetting.config'
  export default {
    name: 'dashboard-template-controller',
    _meta: {
      path: '/dashboardtemplate/controller',
      title: '仪表盘管理'
    },
    mixins: [TabPane],
    components: {
    ...
    data() {
      return {
        ...
        FORM_INFO: DASHBOARD.FORM_INFO,
        buttons: helper.getMainButtons(),
        formdata: {...DASHBOARD.FORM_DATA},
        formOptions: DASHBOARD.FORM_OPTIONS,
        ...
        tableOptions: {
          ...
          columns: [
            ...
            {prop: 'template', label: '路径', minWidth: 200},
    ...
    methods: {
      getData(query) {
        api.dashboardTemplate.getPaged(query).then(data => {
        ...
        let btnHandlers = {
          btnAdd: () => {
            ...
            this.$nextTick(() => {
              ...
              this.formdata = {...DASHBOARD.FORM_DATA}
            })
          },
          btnEdit: (arg) => {
      ...
      sure() {
        this.$refs.form.validate((valid) => {
          if (valid) {
            if (!this.formdata.id) {
              api.dashboardTemplate.add(this.formdata).then(() => {
                this.$message({
                  message: '新增成功!',
                  type: 'success'
                })
              ...
              api.dashboardTemplate.update(this.formdata).then(() => {
                this.$message({
                  message: '编辑成功!',
                  type: 'success'
                })
      ...
      changeIsDefault(index, row) {
        api.dashboardTemplate.statusSwitch(row.id).then(data => {
          this.$notify({
            title: '',
            message: '设置成功',
            type: 'success'
          ...
          if (row.isDefault === 0) {
            this.tableOptions.data.forEach(d => { d.isDefault = 0 })
            row.isDefault = 1
          } else {
            row.isDefault = 0
          }
