...
<script>
  ...
  export default {
    ...
    data() {
      return {
        helper,
        forminfo: {...FOCUS.FORM_INFO},
        formdata: {...FOCUS.FORM_DATA},
        formOptions: Object.assign(FOCUS.FORM_OPTIONS, {
          ico: {
            uploadFile: (params) => {
    ...
    methods: {
      open(row) {
        if (row) {
          this.windowOptions.title = '编辑关注店铺'
          this.formdata = {...row}
          this.$refs.window.open()
        } else {
          this.$refs.window.open()
          this.$nextTick(() => {
            this.windowOptions.title = '新增关注店铺'
            this.$refs.form.resetFields()
            this.formdata = {...FOCUS.FORM_DATA}
          })
        }
      },
      sure() {
        this.$refs.form.validate((valid) => {
          if (valid) {
            if (!this.formdata.id) {
              api.focusStore.add(this.formdata).then(() => {
                this.$message({
                  message: '新增成功!',
                  type: 'success'
                })
              ...
              api.focusStore.update(this.formdata).then(() => {
                this.$message({
                  message: '编辑成功!',
                  type: 'success'
                })
