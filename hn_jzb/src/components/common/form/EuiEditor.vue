    ...
    data() {
      return {
        ...
        editorOption: Object.assign({
          ...
          placeholder: this.placeholder || '产品详情',
          modules: {
            toolbar: {
              container: this.toolbars.length > 0 ? this.toolbars : toolbarOptions,
              handlers: {
                image: (value) => {
                  if (value) {
                    this.uploadType = 'image'
                    // console.log(this.$refs.input.$el.querySelector('.avatar-uploader input'))
                    // console.log(document.querySelector('.avatar-uploader input'))
                    this.$refs.input.$el.querySelector('.avatar-uploader input').click()
      ...
      uploadFile(params) {
        ...
        if (this.uploadType === 'image' && typeof this.uploadPic === 'function') {
          ...
          this.uploadVideo(params.file).then(data => {
            ...
            // let path = '/upload/company/product/video/2e5c7e257c6149b1979ba6c5cb9e1a51.mp4'
            // quill.insertEmbed(length, 'video', helper.getImgUrl(path))
            // // 调整光标到最后
            // quill.setSelection(length + 1)
          })
        }
