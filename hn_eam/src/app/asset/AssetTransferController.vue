<template>
  <eui-layout :regions="REGIONS">
    ...
    <asset-transfer-detail-window ref="detailWindow">
    </asset-transfer-detail-window>
    <asset-transfer-window ref="window"
...
<script>
  ...
  import AssetTransferDetailWindow from './window/detail/AssetTransferDetailWindow'
  import AssetTransferWindow from './window/AssetTransferWindow'
  import fileDownload from 'js-file-download'
  const COLUMNS = [
    {prop: 'transferCode', label: '转移单号', width: 150, fixed: 'left', click: true},
    {prop: 'username', label: '转移后使用人', width: 120, fixed: 'left'},
    {prop: 'departmentName', label: '转移后使用部门', width: 150, align: 'center', fixed: 'left'},
    {prop: 'transferTimeStr', label: '转移时间', width: 150, fixed: 'left'},
    {prop: 'positionName', label: '转移后使用地', width: 150, align: 'center'},
    {prop: 'station', label: '转移后使用工位', width: 150, align: 'center'},
    {prop: 'disposeUserName', label: '领用处理人', width: 100, align: 'center'},
    {prop: 'transferRemark', label: '转移备注', width: 150, align: 'center'},
    {prop: 'assetCode', label: '转移前资产编码', width: 150, align: 'center'},
    {prop: 'assetName', label: '转移前资产名称', width: 150, align: 'center'},
    {prop: 'typename', label: '转移前资产类别', width: 150, align: 'center'},
    {prop: 'assetStatusName', label: '转移前资产状态', width: 150, align: 'center'},
    {prop: 'addTimeStr', label: '转移前入库时间', width: 150, align: 'center'},
    {prop: 'brand', label: '转移前品牌', width: 150, align: 'center'},
    {prop: 'assetSourceName', label: '转移前来源', width: 150, align: 'center'},
    {prop: 'model', label: '转移前型号', width: 150, align: 'center'},
    {prop: 'unit', label: '转移前计量单位', width: 150, align: 'center'},
    {prop: 'place', label: '转移前渠道', width: 150, align: 'center'},
    // {prop: 'positionId', label: '领用前金额', width: 100, align: 'center'},
    {prop: 'buyTimeStr', label: '转移前购入时间', width: 150, align: 'center'},
    {prop: 'usetime', label: '转移前使用期限', width: 150, align: 'center'},
    {prop: 'snnumber', label: '转移前SN号', width: 150, align: 'center'},
    {prop: 'warrantyStartStr', label: '转移前保修起始时间', width: 150, align: 'center'},
    {prop: 'warrantyEndStr', label: '转移前过保时间', width: 150, align: 'center'},
    {prop: 'pictures', label: '转移前图片', width: 150, align: 'center', type: 'picture'},
    {prop: 'remark', label: '转移前资产备注', minWidth: 150, align: 'center'}
  ]
  export default {
    name: 'asset-tranfer',
    _meta: {
      path: '/agency/assets/transfer',
      title: '资产转移'
    },
    mixins: [TabPane],
    components: {
    ...
    data() {
      return {
        ...
        query: {
          ...
          assetSource: null,
          assetStatus: null,
          assetcodeOrAssetnameOrTransfercode: null,
          brand: null,
          department: null,
          model: null,
          page: 1,
    ...
    methods: {
      getData(query) {
        api.assetTranfer.paged(query).then(data => {
      ...
      onBtnClick(btnKey, row) {
        let btnHandlers = {
          ...
          btnEdit: (row) => {
            api.assetTranfer.get(row.id).then(data => {
          ...
          btnDelete: (row) => {
            this.formdata = {...row}
            this.$confirm(`是否删除:${this.formdata.transferCode}？`, '提示', {
              iconClass: 'fa fa-question-circle'
            }).then(() => {
              api.assetTranfer.delete([this.formdata.id]).then(() => {
                this.$message({
                  message: '删除成功!',
                  type: 'success'
                })
          ...
          btnSingleSearch: (arg) => {
            this.query.assetcodeOrAssetnameOrTransfercode = arg
            this.query.page = 1
            this.getData(this.query)
          },
          btnSingleSearchClear: () => {
            this.query.assetcodeOrAssetnameOrTransfercode = null
            this.getData(this.query)
          },
          btnSplitExport0: () => {
            if (this.tableRowsSelect.length === 0) {
              ...
              api.assetTranfer.assetExport({
                type: 2,
                ids: this.tableRowsSelect.toString()
              }).then(res => {
                let blob = new Blob([res.data], {type: 'application/vnd.ms-excel'})
                fileDownload(blob, '转移记录.xls')
              })
            }
          },
          btnSplitExport1: () => {
            api.assetTranfer.assetExport({
              ...
              let blob = new Blob([res.data], {type: 'application/vnd.ms-excel'})
              fileDownload(blob, window.decodeURI(v ? v[1] : '转移记录.xls'))
            })
          }
        }
      ...
      batchDelete() {
        this.$confirm(`是否批量删除选中项？`, '提示', {
          ...
          api.assetTranfer.delete(this.tableRowsSelect).then(() => {
            this.$message({
              message: '删除成功!',
              type: 'success'
            })
      ...
      superSearch(searchinfo) {
        this.query.transferCode = searchinfo.transferCode
        ...
        this.query.transferRemark = searchinfo.transferRemark
