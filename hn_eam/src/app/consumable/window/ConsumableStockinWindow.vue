  ...
  <div>
    <eui-window ref="window"
      ...
      <div slot="btns">
        ...
        <el-button type="primary" size="small" @click="sure('add')">确认入库</el-button>
...
<script>
  ...
  export default {
    name: 'consumable-stockin-window',
    props: {
      positionData: {
        type: Array,
        default: () => []
    ...
    data() {
      return {
        ...
        forminfo: STOCKIN.FORM_INFO,
        formdata: {...STOCKIN.FORM_DATA},
        formOptions: Object.assign(STOCKIN.FORM_OPTIONS, {
          warehouseId: []
        }),
        rules: STOCKIN.RULES,
        windowOptions: {
        ...
        tableRowsSelect: [],
        uploadApi: api.consumableStockin.importTemplate
      }
    },
    methods: {
      open(row) {
        ...
        if (row) {
          this.windowOptions.title = '入库管理编辑'
          this.formdata = row
          this.formdata.inventoryDtoList = []
          this.tableOptions.data = row.stockinConsumableDtoList
          this.$refs.window.open()
        } else {
          this.windowOptions.title = '入库管理新增'
          this.$refs.window.open()
          // this.formdata = {...RECEIVE.FORM_DATA}
          this.tableOptions.data = []
          this.$nextTick(() => {
            this.$refs.form.resetFields()
            this.formdata = JSON.parse(JSON.stringify(STOCKIN.FORM_DATA))
          })
        }
      },
      sure(method) {
        this.$refs.form.validate((valid) => {
          if (this.tableOptions.data.some(d => d.quantity === null || d.quantity === '' || d.totalPrice === null || d.totalPrice === '')) {
            this.$message({
              message: '物料数量和金额不能为空！',
              type: 'warning'
            })
          } else if (valid) {
            this.formdata.inventoryDtoList = this.tableOptions.data.map(row => ({
              consumableId: row.id,
            ...
            if (!this.formdata.id) {
              api.consumableStockin[method](this.formdata).then(() => {
                this.$message({
                  message: '新增成功!',
                  type: 'success'
                })
              ...
              let updateFunName = ''
              if (method === 'zancun') {
                this.formdata.stockinStatus = '0'
                updateFunName = 'updateZancun'
              } else if (method === 'add') {
                this.formdata.stockinStatus = '1'
                updateFunName = 'updateAdd'
              }
              api.consumableStockin[updateFunName](this.formdata).then(() => {
                this.$message({
                  message: '编辑成功',
                  type: 'success'
                })
      ...
      setSelectRows(rows) {
        rows.forEach(row => {
          ...
          this.$set(row, 'totalPrice', null)
          this.tableOptions.data.push(row)
          this.formdata.inventoryDtoList.push(row)
        })
      },
      getTemplate() {
        // window.open(this.serverURI + api.consumableStockin.getTemplate())
        api.consumableStockin.getTemplate().then(res => {
      ...
      getWarehouseData() {
        api.consumableWarehouse.paged({
          ...
          this.formOptions.warehouseId = data ? data.records.map(d => ({
      ...
      tableInputChange(row) {
        if (!row.quantity || !row.totalPrice) {
          row.price = 0
        } else {
          row.price = (row.totalPrice / row.quantity).toFixed(2)
        }
        this.formdata.amount = this.tableOptions.data.reduce((rst, row) => {
          return rst += Number(row.totalPrice)
        }, 0).toFixed(2)
