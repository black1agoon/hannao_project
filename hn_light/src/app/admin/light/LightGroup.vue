<template>
  <eui-layout :regions="REGIONS">
    ...
    <light-group-window ref="window" @reload="tableOptions.reload">
    </light-group-window>
  </eui-layout>
</template>
...
<script>
  ...
  import LightGroupWindow from './window/LightGroupWindow'
  export default {
    name: 'light-group',
    _meta: {
      path: '/light/group',
    ...
    methods: {
        ...
        let btnHandlers = {
          ...
          btnDelete: (row) => {
            this.$confirm(`是否删除:${row.name}？`, '提示', {
              ...
              api.lightGroup.delete(row.id).then(() => {
                this.$message({
                  message: '删除成功!',
                  type: 'success'
                })
      ...
      getData(query) {
        api.lightGroup.paged(query).then(data => {
