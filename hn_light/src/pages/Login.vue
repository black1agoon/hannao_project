...
<script>
  ...
  export default {
      ...
      submit() {
        this.$refs.loginForm.validate((valid) => {
          if (valid) {
            ...
            this.loading = true
            api.public.login(usermsg).then((data) => {
              this.loading = false
              ...
              this.$store.commit('SET_SYSTYPE', 'pac')
              this.$store.commit('SET_AGENCYID', data.agencyId)
    ...
    beforeRouteEnter(to, from, next) {
      next(vm => {
        if (to.query.user && to.query.pwd) {
          vm.formLogin.username = to.query.user
          vm.formLogin.password = to.query.pwd
          vm.submit()
        } else if (to.redirectedFrom && to.redirectedFrom.includes('autoLoginNoPwd')) {
          vm.formLogin.username = 'admin'
          vm.formLogin.password = '666777'
          vm.submit()
        }
      })
    }
...
<style lang="stylus" rel="stylesheet/stylus">
  ...
  .login-page
    /* background-color: rgba(228, 90, 118, 0.49) */
