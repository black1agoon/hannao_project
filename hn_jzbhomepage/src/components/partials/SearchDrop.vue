  ...
  <div class="box-wrapper">
    ...
    <div class="box-inner">
      ...
      <span class="input-icon-right">
        <i v-if="!keyword" class="icomoon-search"></i>
        <i v-else @click="resetInput" class="icomoon-close"></i>
      </span>
      <div class="dropbox" v-if="dropped">
        <slot v-if="filteredList.length > 0">
          <div v-for="(brand, idx) in filteredList" v-if="idx < 1">
            <div class="list-header">支持NFC的机型</div>
            <h3> {{ brand.names.split(',')[0] }}</h3>
            <template v-if="devices[brand.id].length">
              <p v-for="device in devices[brand.id]"> {{ device }} </p>
            </template>
