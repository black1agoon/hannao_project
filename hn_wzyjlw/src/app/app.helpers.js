...
export default {
  ...
  getTableButtonsByKeys(buttons, keys) {   // 根据指定的key获取相应buttons
    return buttons.filter(btn => keys.includes(btn.btnKey))
  },
  getTabsTableButtons(buttons, tabName, filter = []) {  // tabs中获取buttons
    return buttons.filter(btn => {
      return btn.btnKey.indexOf(tabName) !== -1 && !filter.includes(btn.btnKey)
    })
  },
