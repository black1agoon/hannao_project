    ...
    methods: {
      ...
      getTreeData() {
        api.department.getTree().then((data) => {
          this.treeOptions.data = this.formOptions.depId = this.SEARCH_OPTIONS.depId = helper.getTreeData(data, 'childDepartmentDtoList', 'name')
        })
      },
      ...
      searchFunctions(method, keyword) {
        let funs = {
          openDepChose: () => {
            this.$refs.depWindow.open('search')
          },
          search: (keyword) => {
            this.query.page = 1
            this.query.mobile = keyword.mobile
            this.query.realname = keyword.realname
            this.query.username = keyword.username
            this.query.depId = keyword.depId
            this.getData(this.query)
          }
        }
