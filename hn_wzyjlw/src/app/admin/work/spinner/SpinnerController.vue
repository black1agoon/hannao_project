...
<script>
  ...
  import {CONTROLLER} from './spinner.config'

  export default {
    name: 'spinner-controller',
    components: {
      EuiTable,
      EuiWindow,
      EuiInput,
    ...
    methods: {
      ...
      onBtnClick(btnKey, arg) {
        let btnHandlers = {
          ...
          btnspinnerDelete: (arg) => {
            this.formdata = {...arg}
            this.$confirm(`是否删除:${this.formdata.name}？`, '提示', {
              iconClass: 'fa fa-question-circle'
            }).then(() => {
              api.spinner.delete(this.formdata.id).then(() => {
                this.$message({
                  message: '删除成功!',
                  type: 'success'
                })
      ...
      sure() {
        this.$refs.form.validate((valid) => {
          if (valid) {
            if (!this.formdata.id) {
              api.spinner.add(this.formdata).then(() => {
                this.$message({
                  message: '新增成功!',
                  type: 'success'
                })
              ...
              api.spinner.update(this.formdata).then(() => {
                this.$message({
                  message: '编辑成功!',
                  type: 'success'
                })
