...
<script>
  ...
  export default {
    name: 'multiple-stations-chose-window',
    components: {
      EuiInput,
      EuiLayout,
      EuiTree,
    ...
    computed: {
      getChooseList() {
        return this.multipleSelectionAll.map(s => s.stationCode).join(',')
      }
    },
    data() {
      return {
    ...
    methods: {
      ...
      getData(query) {
        return api.station.paged(query).then(data => {
      ...
      sure() {
        if (this.multipleSelectionAll.length > 0) {
          this.$parent.$emit('multiplestation', this.from, this.multipleSelectionAll)
